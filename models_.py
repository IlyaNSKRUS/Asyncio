...
async def init_orm():
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.drop_all)             # Очистка таблицы
        await conn.run_sync(Base.metadata.create_all)           # Создание таблицы __tablename__ = 'swapi_people'

async def close_orm():
    await engine.dispose()                                      # Закроет ORM
